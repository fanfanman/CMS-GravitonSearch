...
def plotsingle(signalHists, dyHist, label):
	...
	if resolution:	
		if muon:
			canv.Print("rawDataPlots/EtaHist_combine_%sMuon_Con.pdf"%label)
		else:	
			canv.Print("rawDataPlots/EtaHist_combine_%sElectron_Con.pdf"%label)
	else:
		canv.Print("rawDataPlots/EtaHist_combine_%s_Con.pdf"%label)
