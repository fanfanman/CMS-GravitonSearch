...
# function to get specific bin content of hist
# given binedges
def getBinContent(hist, edges):
	
	rethist = []
	
	lowedge = edges[0]
	highedge = edges[1]
	thishist = TH1F("part", "part", 20, lowedge, highedge)
	for i in range(1, hist.GetSize()-1):
		if (hist.GetBinCenter(i) > highedge):
			rethist.append(thishist.Clone())
			if (highedge == edges[-1]):
				return rethist
			else:
				lowedge = highedge
				highedge = edges[edges.index(lowedge)+1]
				thishist = TH1F("part", "part", 20, lowedge, highedge)
				print lowedge, highedge
		# print hist.GetBinCenter(i), hist.GetBinContent(i)
		thishist.Fill(hist.GetBinCenter(i), hist.GetBinContent(i))
...
def plotsingle(lambdaT):
	...
	binedges = [1000, 2000, 3000, 4000, 5000, 6000, 7000]
	sig = getBinContent(signalHist, binedges)
	dy = getBinContent(dyHist, binedges)
	...
	plotPad.cd()
	plotPad.SetLogy()
	plotPad.DrawFrame(120,2e-7,7000,2.5e3,";mass [GeV];Events")

	# plotting
	for (i, histi) in enumerate(sig):
	...
	# draw legend
	leg = TLegend(0.4, 0.51, 0.94, 0.92,"","brNDC")
	leg.SetFillColor(10)
	leg.SetFillStyle(0)
	leg.SetLineColor(10)
	...
	if resolution:
		leg.AddEntry(dyHist,"DY smeared (#Lambda = 4000)","l")
		for (i, histi) in enumerate(sig):
			leg.AddEntry(histi,"DY x ADD smeared (bin edge %d - %d)"%(1000*(i+1), 1000*(i+2)),"l")
	else:
	...
	if resolution:	
		if muon:
			canv.Print("plots/changebin.pdf")
		else:	
			canv.Print("plots/changebin.pdf")
	else:
		canv.Print("plots/changebin.pdf")
