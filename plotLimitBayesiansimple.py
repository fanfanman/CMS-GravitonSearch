...
def getLimits(file_name):
    ...
    for quantile in tree:
        thisl = tree.GetLeaf("limit").GetValue()
        limits.append(thisl)
        #print tree.limit
 
    result = sum(limits)/len(limits)
    print result
    return result
...
def plotUpperLimits(model, lambdas, helicity):
    ...
    for i in range(N):
        file_name = "./%sdataCards/ee_limit_multibin/higgsCombineTest.BayesianSimple.mH%d.123456.root"%(model, lambdas[i])
        ...
        if model == "ADD":
            ...
            median.SetPoint(    i,    lambdas[i]/1000, limit)# median
    ...
    legend.AddEntry(median, "BayesianSimple expected %s%s"%(model, helicity),'L')
    #legend.AddEntry(green, "#pm 1 std. deviation",'f')
    #legend.AddEntry(yellow,"#pm 2 std. deviation",'f')
    legend.Draw()

    if XSec: 
        c.SaveAs("%slimits/%sLimit_ee_XSecVsMultibin.png"%(model, model))
    else: 
        c.SaveAs("%slimits/%sLimit_ee_Multibin.png"%(model, model))
    c.Close()
