...
def templateFormat(sigYieldList, dyYieldList):
	
	nbin = len(sigYieldList)
	curr = multibintemplate%(nbin, nbin)
...
def writeDatacard(model, sigYield, dyYield, lambdaT, helicity):
	...
	fout = open(fname, "w")
	fout.write(templateFormat(sigYield, dyYield))
	fout.close()
	return fname
...
def executeDatacard(model, fname, lambdaT, label):
	...
	# BayesianSimple calculation
        '''combine_command = "combine -M BayesianSimple %s -m %d -t 100 --noDefaultPrior=0"%(fname, lambdaT)
        print ">>> command: " + combine_command
        ...
        rf = "higgsCombineTest.BayesianSimple.mH%d.123456.root"%(lambdaT)
        mvfile = subprocess.Popen("mv ./%s ./%sdataCards/ee_limit_multibin%s/%s"%(rf, model, label, rf), shell=True)
        print ">>> file moved"
        retval = p.wait()'''
...
def main(argv):
	...
	#Mbins = [1800 + i * 100 for i in range(31)]
        #Mbins.append(10000) # bw100
        #Mbins = [1800 + i * 200 for i in range(16)] # bw200
	#Mbins.append(10000)
	...
	for i in range(len(lambdas)):
		for j in range(len(heli)):
			fout = writeDatacard(model, hhNum[i][j], dyNum[i], lambdas[i], heli[j])
