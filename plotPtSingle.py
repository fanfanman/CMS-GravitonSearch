...
def plotsingle(signalHist, dyHist, label, lambdaT):
	...
	if resolution:	
		if muon:
			canv.Print("rawDataPlots/PtHist_LambdaT%d_%sMuon_Con.pdf"%(lambdaT, label))
		else:	
			canv.Print("rawDataPlots/PtHist_LambdaT%d_%sElectron_Con.pdf"%(lambdaT, label))
	else:
		canv.Print("rawDataPlots/PtHist_LambdaT%d_%s_Con.pdf"%(lambdaT, label))
