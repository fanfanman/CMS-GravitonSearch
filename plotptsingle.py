...
def readHisto(lambdaT):
	...
	signalHist1.Scale(0.02)
	signalHist2.Scale(0.02)
	dyHist1.Scale(0.02)
	dyHist2.Scale(0.02)
...
def plotsingle(signalHist, dyHist, label, lambdaT):
	...
	if resolution:	
		if muon:
			canv.Print("plots/PtHist_LambdaT%d_%sMuon.pdf"%(lambdaT, label))
		else:	
			canv.Print("plots/PtHist_LambdaT%d_%sElectron.pdf"%(lambdaT, label))
	else:
		canv.Print("plots/PtHist_LambdaT%d_%s.pdf"%(lambdaT, label))
