...
def plotsingle(signalHists, dyHist, label):
	...
	plotPad.cd()
	plotPad.SetLogy()
	plotPad.DrawFrame(-2.4,1.0e4,2.4,1.0e5,";Eta (%s);Events"%label)

	# plotting
	for (i, histi) in enumerate(signalHists):
		histi.SetLineColor(i+2)
		histi.Draw("histsame")
		print histi.GetBinContent(1)
	...
	if resolution:
		for (i, histi) in enumerate(signalHists):
			leg.AddEntry(histi, "Drell-Yan x ADD smeared (#Lambda = %s)"%(2000+2000*(i+1)),"l")
	...
	if resolution:	
		if muon:
			canv.Print("plots/EtaHist_combine_%sMuon.pdf"%label)
		else:	
			canv.Print("plots/EtaHist_combine_%sElectron.pdf"%label)
	else:
		canv.Print("plots/PtHist_combine_%s.pdf"%label)
