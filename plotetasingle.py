...
def getMassDistroSignal(name, mass):
	...
	for massBin in massBins:
		temp1, temp2 = getMassHisto("rootfiles/ADD_LambdaT%d_%s_13TeV-pythia8_cff_1.root"%(mass,massBin))
		result1.Add(temp1)
		result2.Add(temp2)
...
def getMassDistroDY():
	...
	for massBin in massBins:
		temp1, temp2 = getMassHisto("rootfiles/DYTo%s_%s_13TeV-pythia8_cff_1.root"%(leptype, massBin))
		result1.Add(temp1)
		result2.Add(temp2)
...
def readHisto(lambdaT):
	...
	'''ret1 = TH1F("diff", "diff", 48, -2.4, 2.4)
	ret2 = TH1F("diff", "diff", 48, -2.4, 2.4)

	for i in range(1, dyHist1.GetSize()-1):
		ret1.Fill(signalHist1.GetBinCenter(i), signalHist1.GetBinContent(i)-dyHist1.GetBinContent(i))
		ret2.Fill(signalHist2.GetBinCenter(i), signalHist2.GetBinContent(i)-dyHist2.GetBinContent(i))
	'''
...
def plotsingle(signalHist, dyHist, label, lambdaT):
	...
	if resolution:	
		if muon:
			canv.Print("plots/EtaHist_LambdaT%d_%sMuon.pdf"%(lambdaT, label))
		else:	
			canv.Print("plots/EtaHist_LambdaT%d_%sElectron.pdf"%(lambdaT, label))
	else:
		canv.Print("plots/EtaHist_LambdaT%d_%s.pdf"%(lambdaT, label))
